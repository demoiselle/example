...
angular
        ...
        .factory('client', ["$q", "$window", "$http", "$log", "plugins", function ($q, $window, $http, $log, plugins) {
                ...
                function configure(operation, values, baseUrl) {
                    ...
                    headers['content-type'] = body ? values.contentType : 'application/json';
...
angular.module('sw.ui.directives')
        ...
        .factory('truncation', function () {
            ...
            function truncate(root, opts) {
                ...
                if (excess < 0 || !excess && !opts.truncated)
                    return;
...
angular.module("sw.ui.md").run(["$templateCache", function ($templateCache) {
        $templateCache.put("auth.html", "<!DOCTYPE html> <html lang=\"en\"> <head> <meta charset=\"UTF-8\"> <title>OAuth 2.0 Callback</title> </head> <script>// from https://github.com/swagger-api/swagger-ui/blob/master/dist/o2c.html\n\n    var qp;\n\n    if (window.location.hash) {\n        qp = location.hash.substring(1).replace(/^\\//, \'\');\n    } else {\n        qp = location.search.substring(1);\n    }\n\n    qp = qp ? JSON.parse(\'{\"\' + qp.replace(/&/g, \'\",\"\').replace(/=/g, \'\":\"\') + \'\"}\',\n        function (key, value) {\n            return key === \'\' ? value : decodeURIComponent(value)\n        }\n    ) : {};\n\n    if (window.opener) {\n        window.opener.onOAuthFinished(qp);\n        window.close();\n    }</script> </html>");
